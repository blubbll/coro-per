...
//hazard icon svg by iconmonstr to prevent unicode turning to emoji
...
applyData = infected => {
  /////// ppl infected by https://world-statistics.org/
  ...
  const perc = (infected * 100) / ppl;

  //update window infected on real update only
  infected !== window.infected && [
    console.debug(`${new Date()}: updated infected`, infected),
    (window.infected = infected)
  ];
  $(
    "#infected"
  ).innerHTML = `${perc}%<br/><span class="small">(${haz}&#8202;${infected} of ${ppl} people)</span>`;
};

//update ppl infected
const updateLocal = setInterval(() => applyData(window.infected), 1399);
